...
- **[ACTIVE] Tier 1 Release Packaging** (`tier1-release-packaging`)
  ...
  - **LATENT CODE ISSUE (tracked, not blocking v1):** the hu16 PNG encode/decode is inconsistent across the codebase. Shipped training/eval PNGs use `u16 = round(HU*10) + 32768` (decode `(u16−32768)*0.1`, verified vs on-disk bytes; `zoo/data.py`, `eval_external.py`, `zoo/predict.py`, `phase5_big_run.py` all use this). But `scripts/preprocessing/phase2_preprocess_*.py` encode with `round(HU + 32768)` (×1, no scale) and `scripts/phase4_monitor.py` decodes with ×1 — these two DISAGREE with the shipped data. `docs/DATA_SOURCES.md` also mislabels the encoding as `(HU + 32768) * 10`. Does NOT affect the released model (train+eval self-consistent at ×0.1). Must be reconciled before any future preprocessing run or the next model trains on mis-scaled HU.
